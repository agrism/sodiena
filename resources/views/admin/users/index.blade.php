...
@section('title_topbar', 'ŠODIENA.LV :: LIETOTĀJU REĢISTRS UN LOMAS')
...
<div class="max-w-7xl xl:max-w-[1400px] 2xl:max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-8">
    ...
    <!-- Filter & Search Bar -->
    @php
        $isUserSearchActive = !empty($search);
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs">
        <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-col sm:flex-row gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                <button 
                    type="submit" 
                    class="px-4 py-2.5 rounded-xl bg-slate-900 text-white text-xs sm:text-sm font-bold hover:bg-slate-800 transition-all cursor-pointer">
                    {{ __('Filter') }}
                </button>
